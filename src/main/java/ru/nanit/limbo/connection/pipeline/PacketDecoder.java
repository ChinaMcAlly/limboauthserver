...
public class PacketDecoder extends MessageToMessageDecoder<ByteBuf> {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buf, List<Object> out) throws Exception {
        ...
        if (packet != null) {
            Logger.debug("Received packet "+packet.toString()+"[0x"+Integer.toHexString(packetId)+"]");
            try {
                packet.decode(msg, version);
            } catch (Exception e) {
                e.printStackTrace();
                Logger.warning("Cannot decode packet 0x"+Integer.toHexString(packetId)+": "+ e.getMessage());
            }
