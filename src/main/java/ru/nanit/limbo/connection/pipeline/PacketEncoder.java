...
public class PacketEncoder extends MessageToByteEncoder<Packet> {
    ...
    protected void encode(ChannelHandlerContext ctx, Packet packet, ByteBuf out) throws Exception {
        ...
        if (packetId == -1) {
            Logger.warning("Undefined packet class: "+packet.getClass().getName()+"[0x"+Integer.toHexString(packetId)+"]");
            return;
        }
        ...
        try {
            packet.encode(msg, version);
             Logger.debug("Sending "+packet.toString()+"[0x"+Integer.toHexString(packetId)+"] packet ("+msg.readableBytes()+" bytes)");
        } catch (Exception e) {
